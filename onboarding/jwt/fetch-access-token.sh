...
now_seconds=$(date +%s)
if [ ! -f $TOKENIZATION_ACCESS_TOKEN_EXPIRY_FILE ]; then
  expiry=0
else
  expiry=$(cat $TOKENIZATION_ACCESS_TOKEN_EXPIRY_FILE)
fi
...
if [ $(($expiry - $now_seconds)) -lt 1800 ]; then
  ...
  echo $token_response | jq -r '.access_token' > $TOKENIZATION_ACCESS_TOKEN_FILE
  expires_in=$(echo $token_response | jq -r '.expires_in')
  expiry=$(($now_seconds + $expires_in))
  echo $expiry > $TOKENIZATION_ACCESS_TOKEN_EXPIRY_FILE
fi
